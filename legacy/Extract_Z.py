...
def main(args):
    ...
    if args.dual_omics:
        ...
        #omics1_df.to_csv(os.path.join(args.output_dir,"omics1_test_output.txt"), sep='\t', index=False)
        #omics2_df.to_csv(os.path.join(args.output_dir,"omics2_test_output.txt"), sep='\t', index=False)
    ...
    # Save z1 and z2
    z1_df = pd.DataFrame(z1_array, columns=[f"Z1_{i}" for i in range(z1_array.shape[1])])
    z2_df = pd.DataFrame(z2_array, columns=[f"Z2_{i}" for i in range(z2_array.shape[1])])

    z1_df.to_csv(os.path.join(args.output_dir,"z1_latent_vectors.txt"), sep='\t', index=False)
    z2_df.to_csv(os.path.join(args.output_dir,"z2_latent_vectors.txt"), sep='\t', index=False)
